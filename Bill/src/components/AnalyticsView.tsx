      ...
      <header className="sticky top-0 bg-white border-b border-slate-100 px-4 py-4 md:px-8 z-10 flex items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          <button 
            ...
            className="p-1.5 hover:bg-slate-50 text-slate-500 hover:text-slate-800 rounded-lg cursor-pointer transition-colors"
          >
            <ArrowLeft size={18} />
          </button>
          <div>
            <h1 className="font-sans font-bold text-lg md:text-xl text-slate-800">
              {isId ? 'Analisis Kinerja Bisnis' : 'Business Performance Analytics'}
            </h1>
            <p className="text-xs text-slate-400">
              {isId ? 'Tinjauan mendalam performa tagihan Anda' : 'Deep insights into your billing performance'}
            </p>
          </div>
        </div>
