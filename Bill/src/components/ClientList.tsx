  ...
  return (
    <div className="flex-1 bg-[#F8FAFC] pb-24 lg:pb-8">
      ...
      <header className="sticky top-0 bg-white border-b border-slate-100 px-4 py-4 md:px-8 z-10 flex items-center justify-between gap-4">
        <div>
          <h1 className="font-sans font-bold text-xl md:text-2xl text-slate-800">{t.clients}</h1>
          <p className="text-xs text-slate-400 mt-0.5">
            {profile.language === 'id' 
              ? `Total ${clients.length} klien terdaftar` 
              : `Total ${clients.length} registered clients`
            }
          </p>
        </div>
